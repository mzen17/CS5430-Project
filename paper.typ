...
The variational autoencoder uses 3 layers for the encoding model of dimensions 794x256, 256x64, and 64x8. Notice the latent dimension is $8$. The decoder uses 3 linear layers of 18x64, 64x256 and 256x784, with ReLU after the first two layers and a sigmoid mean output. The reported experiments use a fixed decoder log variance of $0$, i.e. variance $I$. Initially, the model was tested with learned variance, but training became unstable and the KL term produced NaNs. However, all loss calculations still use the decoder log variance term.
...
We found that using randomly initialized vectors for the starting $z$ resulted in the model having poor performance, so we trained the EM-approximation setup with $z$ initialized from the encoding model with its weights frozen. We did 150 sampling steps.

=== M-step Update
Using the finalized latent $z$, we update our decoder. Using the batch group $b$, we take the negative Gaussian likelihood of our batch as our loss. The decoder is updated using the Adam optimizer on the loss. Our M-step count was $3$, so we did 3 gradient descent steps per E-step.
...
$log p(x)$ was approximated on 50 test images using 10000 prior samples of $z$. The table shows the first 8 examples and the mean over all 50:
#table(
...
The reported models were trained on an NVIDIA RTX 5070 Ti. The LEM model took around 3 minutes to train for 10000 steps while the VAE took about 20 seconds to train.
...
The more interesting difference is in the latent representations. The VAE latents appear more clustered by digit class, which is expected because the encoder directly learns a smooth amortized map from images to latent variables. The Langevin latents from the z-bank are less cleanly clustered. This makes sense because they are not produced by an encoder network; they are persistent latent variables refined by posterior sampling. 
...
== Conclusion
The experiment shows that approximate EM with Langevin sampling is a workable alternative to standard VAE training on MNIST, but it is not clearly better in this setting. The EM-trained model produced comparable samples and nearly identical estimated test likelihoods, while requiring substantially more computation. Overall, the VAE remains the more practical method for this small image generation task.
